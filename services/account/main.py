"""Account service – FastAPI with alert endpoints."""
import socket
import uuid
...
# Alert type distribution (more frequent vs rarer)
WARNING_TYPES = [
    ("timeout", 30),
    ("error_rate", 25),
    ("latency_high", 20),
...
def alert_summary(hours: Literal[12, 24] = Query(12, description="12 or 24")):
    """Alert summary for the last 12h or 24h."""
    now = datetime.utcnow()
...
def generate_one_alert(warning_type: str | None = None):
    """Generates one unique alert from this service (account)."""
    try:
        now = datetime.utcnow()
...
def generate_bulk_alerts(
    ...
    """Generates up to 200 alerts from this service (account), various types."""
    try:
        now = datetime.utcnow()
        weights = [w for _, w in WARNING_TYPES]
        total_w = sum(weights)
        # put_item in loop instead of batch_writer (avoids batch issues)
